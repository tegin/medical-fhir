    ...
    var QuestionnaireRenderer = BasicRenderer.extend({
        ...
        _renderView: function() {
            ...
            _.each(this.state.data, function(data) {
                ...
                var field_type = self._itemType2Object(data.data.question_type);
                data.data.field_type = field_type;
                if (data.data[field_name] === undefined && field_type !== undefined) {
                    ...
                    if (field_type === "radio") {
                        var selection = [];
                        _.each(data.data.selection_options.split(";"), function(item) {
                ...
                if (data.data.technical_name) {
                    ...
                    fieldInfo[response][data.data.technical_name] =
                        data.data[field_name];
                }
            });
            _.each(this.state.data, function(data) {
                var element = $(
                    qweb.render("medical_clinical_questionnaire.item", {
                        widget: self,
                        data: data,
                    })
                );
                ...
                if (Widget !== undefined) {
                    ...
                    if (data.data.invisible_condition) {
                        if (
                            py.eval(data.data.invisible_condition, fieldInfo[response])
                        ) {
                            return;
                        }
                    ...
                    if (data.data.readonly_condition) {
                        if (
                            mode === "edit" &&
                            py.eval(data.data.readonly_condition, fieldInfo[response])
                        ) {
                            mode = "readonly";
                        }
