...
class MedicalDiagnosticReport(models.Model):
    ...
    def _compute_report_bokeh_chart(self):
        if self.compute_graph:
            bokeh, html = self._get_bokeh_information()
    ...
    def _get_image_from_bokeh(self):
        if self.compute_graph:
            bokeh_image, html_chart = self._get_bokeh_information(True)
            self.write({"html_chart": html_chart, "bokeh_image": bokeh_image})

    def registered2final_action(self):
        self._get_image_from_bokeh()
    ...
    def show_graphs(self):
        ...
        self.flush()
        self._compute_report_bokeh_chart()
        return {
