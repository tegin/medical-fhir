...
class TestMedicalEncounter(TransactionCase):
    ...
    def test_action_view_external_medical_product_request_order_ids(self):
        self.assertEqual(self.encounter.external_product_request_order_count, 1)
        action = self.encounter.action_view_external_medical_product_request_order_ids()
        self.assertEqual(action["res_id"], self.external_product_request_order.id)
        self.assertEqual(action["context"]["default_patient_id"], self.patient.id)
        self.assertEqual(action["context"]["default_encounter_id"], self.encounter.id)
        self.assertEqual(action["context"]["default_category"], "discharge")

    def test_action_view_internal_medical_product_request_order_ids(self):
        self.assertEqual(self.encounter.internal_product_request_order_count, 1)
        action = self.encounter.action_view_internal_medical_product_request_order_ids()
        self.assertEqual(action["res_id"], self.internal_product_request_order.id)
        self.assertEqual(action["context"]["default_patient_id"], self.patient.id)
        self.assertEqual(action["context"]["default_encounter_id"], self.encounter.id)
